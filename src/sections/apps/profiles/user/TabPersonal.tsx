...
import useSingleProfile from 'sections/apps/_hooks/getSingleProfile';
import useProfileUpdateMutation from '../_hooks/useProfileUpdateMutation';
...
const TabPersonal = () => {
  ...
  const errorHandler = (error: any) => {
    toast.error('Failed in updated!');
  };

  const maxDate = new Date();
  maxDate.setFullYear(maxDate.getFullYear() - 18);
  const inputRef = useRef();
  // getting signle profile data
  const { data: singleProfile, isLoading } = useSingleProfile();
  // updating profile mutation
  const profileUpdate = useProfileUpdateMutation(successHandler, errorHandler);

  let profileAttributes;
  // Check if singleProfile exists and has the expected structure
  if (singleProfile && Array.isArray(singleProfile.data) && singleProfile.data.length > 0) {
    profileAttributes = singleProfile.data[0].attributes;
  } else {
    console.log('Invalid or empty singleProfile data');
  }
  if (isLoading) {
    ...
    <MainCard content={false} title="Personal Information" sx={{ '& .MuiInputLabel-root': { fontSize: '0.875rem' } }}>
      <Formik
        initialValues={{
          ...
          contact: profileAttributes?.PersonalPhone,
          designation: profileAttributes?.Designation,
          address: profileAttributes?.Address1,
          address1: profileAttributes?.Address2,
          country: profileAttributes?.Country,
          state: profileAttributes?.State,
          skill: [
            'Adobe XD',
            'Angular',
            'Corel Draw',
        ...
        validationSchema={Yup.object().shape({
          ...
          address: Yup.string().min(20, 'Address to short.').required('Address is required'),
          country: Yup.string().required('Country is required'),
          state: Yup.string().required('State is required'),
          Bio: Yup.string().min(20, 'Not shoulde be more then 150 char.')
        })}
        onSubmit={(values, { setErrors, setStatus, setSubmitting }) => {
          ...
          const profilePayload = {
            data: {
              ...
              Bio: values.Bio,
              PersonalPhone: values.contact,
              countryCode: values.countryCode,
              Country: values.country
            }
          };
          profileUpdate.mutateAsync(profilePayload);
          try {
            openSnackbar({
              open: true,
      ...
      >
        {({ errors, handleBlur, handleChange, handleSubmit, isSubmitting, setFieldValue, touched, values }) => (
          <form noValidate onSubmit={handleSubmit}>
            ...
            <Divider />
            <Box sx={{ p: 2.5 }}>
              <InputLabel htmlFor="personal-addrees2">Bio</InputLabel>
              <TextField
                multiline
                rows={5}
                fullWidth
              ...
              {touched.Bio && errors.Bio && (
                <FormHelperText error id="personal-Bio-helper">
                  {errors.Bio}
                </FormHelperText>
              )}
